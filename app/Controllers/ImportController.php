...
class ImportController extends BaseController
{
    ...
    public function products($file, $idCompany){
        $errors = [];
        ...
        try {
            ...
            for ($rowIndex = 2; $rowIndex <= $largestRowNumber; $rowIndex++) {
                ...
                if (!empty($sheet->getCell('A'.$rowIndex)->getValue())) {
                    $code = explode('-', trim($sheet->getCell('A'.$rowIndex)->getValue()));
                    $codeCategoria = $code[0];
                    $categoria = $tableProviders->where(['code' => $codeCategoria])->asObject()->first();
                    if (is_null($categoria)) {
                        array_push($error, "No existe código para 'Categoria'.  Fila # " . $rowIndex);
                    }
                } else {
                    array_push($error, "El campo Categoria es obligatorio.  Fila # " . $rowIndex);
                }
                ...
                if (!empty($sheet->getCell('C'.$rowIndex)->getValue()) && trim($sheet->getCell('C'.$rowIndex)->getValue()) != '') {
                    ...
                    $linea = $tableMaterials->where(['code' => $codeLinea])->asObject()->first();
                    if (is_null($linea)) {
                        array_push($error, "No existe código para 'Linea'.  Fila # " . $rowIndex);
                    }
                } else {
                    array_push($error, "El campo Linea es obligatorio.  Fila # " . $rowIndex);
                }
                ...
                if (empty($sheet->getCell('G'.$rowIndex)->getValue()) && trim($sheet->getCell('G'.$rowIndex)->getValue()) == '') {
                    ...
                    array_push($error, "El campo Valor es obligatorio.  Fila # " . $rowIndex);
                } 
                ...
                if (empty($sheet->getCell('H'.$rowIndex)->getValue()) && trim($sheet->getCell('H'.$rowIndex)->getValue()) == '') {
                    ...
                    array_push($error, "El campo Costo es obligatorio.  Fila # " . $rowIndex);
                }
            }
            if (count($error) > 0) {
                return redirect()->back()->with('errors', implode('<br>', $error));
            }
    ...
    public function download($name){
        if($name == 'Inventario'){
            ...
            ->select(['code', 'name'])
            ->where(['tax_iva' => 'R', 'kind_product_id' => NULL])
            ->asObject()->get()->getResult();
            // Columnas A8 hasta T8
            $fileName = 'PlantillaInventario.xlsx';
            ...
            $spreadsheet->setActiveSheetIndex(0)->setCellValue('A1', 'Código del producto')->getStyle('A1')->getFont()->setBold(true)->getColor()->setARGB(\PhpOffice\PhpSpreadsheet\Style\Color::COLOR_WHITE);
            $spreadsheet->setActiveSheetIndex(0)->setCellValue('B1', 'Descripción del producto')->getStyle('B1')->getFont()->setBold(true)->getColor()->setARGB(\PhpOffice\PhpSpreadsheet\Style\Color::COLOR_WHITE);
            $spreadsheet->setActiveSheetIndex(0)->setCellValue('C1', 'Cantidad')->getStyle('C1')->getFont()->setBold(true)->getColor()->setARGB(\PhpOffice\PhpSpreadsheet\Style\Color::COLOR_WHITE);
            $spreadsheet->getActiveSheet()->getStyle('A1:C1')->applyFromArray($styleArray, false);
            ...
            foreach ($products as $val){
                ...
                $spreadsheet->getActiveSheet()->getStyle('A'.$rows.':C'.$rows)->applyFromArray($styleArray1, false);
            $rows++;
            }
